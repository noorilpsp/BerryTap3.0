/**
 * Run drizzle/0007_location_stations.sql (no psql required).
 * Loads .env.local via Next.js env config.
 *
 * Usage: npm run db:migrate:0007
 */
...
async function run() {
  const sql = neon(url);
  const migrationPath = join(process.cwd(), "drizzle/0007_location_stations.sql");
