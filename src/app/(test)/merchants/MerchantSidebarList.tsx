...
async function prefetchImages(href: string) {
  ...
  try {
    const url = new URL(href, window.location.href);
    await fetch(`/api/prefetch-images${url.pathname}`, {
      priority: "low",
    });
  } catch (error) {
    // swallow; prefetch is best-effort
    console.error("prefetch-images failed", error);
  }
}

export function MerchantSidebarList({ merchants }: { merchants: Merchant[] }) {
  useEffect(() => {
    // kick off prefetch for the first few visible links to match category behavior
    const first = merchants.slice(0, 8);
    first.forEach((m) => {
      void prefetchImages(`/merchants/${m.id}`);
    });
  }, [merchants]);
