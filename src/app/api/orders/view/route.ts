...
import { buildOrdersView } from "@/lib/orders/buildOrdersView";
import { posFailure, posSuccess, toErrorMessage } from "@/app/api/_lib/pos-envelope";

export const runtime = "nodejs";

/**
 * GET /api/orders/view?locationId=<uuid>
 * Returns full orders read model for the location. Read-only.
...
export async function GET(request: NextRequest) {
  try {
    ...
    return posFailure(
      "INTERNAL_ERROR",
      toErrorMessage(error, "Internal server error - Failed to load orders view"),
      { status: 500 }
    );
  }
}
