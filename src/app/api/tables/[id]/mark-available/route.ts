...
import { markTableCleaningDone } from "@/app/actions/tables";
import { posFailure, posSuccess, requireIdempotencyKey, toErrorMessage } from "@/app/api/_lib/pos-envelope";

export const runtime = "nodejs";

/**
 * POST /api/tables/[id]/mark-available
 * Records table_cleaned on the latest closed session so the table returns to available.
 */
export async function POST(
  ...
  try {
    ...
    const result = await markTableCleaningDone(table.location.id, table.id);
    if (!result.ok) {
      return posFailure("CONFLICT", result.error ?? "Failed to mark table available", {
        status: 409,
        correlationId: idempotencyKey,
      });
    }

    return posSuccess({ tableId: table.id }, { correlationId: idempotencyKey });
  } catch (error) {
    console.error("[POST /api/tables/[id]/mark-available] Error:", error);
    return posFailure(
      "INTERNAL_ERROR",
      toErrorMessage(error, "Internal server error - Failed to mark table available"),
