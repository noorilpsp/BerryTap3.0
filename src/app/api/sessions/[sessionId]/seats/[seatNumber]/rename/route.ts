...
export async function PUT(
  ...
  try {
    ...
    const newSeatNumber =
      typeof body.newSeatNumber === "number" && Number.isFinite(body.newSeatNumber)
        ? Math.floor(body.newSeatNumber)
        : typeof body.name === "string"
          ? parseInt(body.name, 10)
          : NaN;
    if (!Number.isFinite(newSeatNumber) || newSeatNumber < 1) {
      return posFailure("BAD_REQUEST", "newSeatNumber is required and must be a positive number", {
        status: 400,
    ...
    const result = await renameSeat(sessionId, parsedSeatNumber, newSeatNumber);
    if (!result.ok) {
      return posFailure(
        "BAD_REQUEST",
        result.error ?? "Failed to rename seat",
    ...
    return posFailure(
      "INTERNAL_ERROR",
      toErrorMessage(error, "Internal server error - Failed to rename seat"),
