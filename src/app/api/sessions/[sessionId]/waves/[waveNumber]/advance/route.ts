...
import { advanceWaveStatus } from "@/domain";
 ...
 * Body: { toStatus: "preparing"|"ready"|"served", eventSource? }
 */
export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ sessionId: string; waveNumber: string }> }
) {
  const ROUTE = "POST /api/sessions/[sessionId]/waves/[waveNumber]/advance";
  ...
  try {
    ...
    const toStatus = body.toStatus as "preparing" | "ready" | "served" | undefined;
    const eventSource = normalizeEventSource(body.eventSource);
    if (toStatus !== "preparing" && toStatus !== "ready" && toStatus !== "served") {
      return posFailure(
        "BAD_REQUEST",
        'Invalid toStatus. Use "preparing", "ready", or "served".',
    ...
    const result = await advanceWaveStatus(sessionId, parsedWaveNumber, toStatus, {
      eventSource,
    });
      ...
      const statusCode =
        result.reason === "session_not_found" || result.reason === "order_not_found"
          ...
          : result.reason === "unauthorized"
            ? 403
            : result.reason === "advance_partial_failure"
              ? 409
              : 400;
      const code =
        statusCode === 404
    ...
    console.error("[POST /api/sessions/[sessionId]/waves/[waveNumber]/advance] Error:", error);
    return posFailure(
      "INTERNAL_ERROR",
      toErrorMessage(error, "Internal server error - Failed to advance wave status"),
