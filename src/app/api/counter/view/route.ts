...
/**
 * GET /api/counter/view?locationId=<uuid>
 * Returns full counter read model (menu data) for the location. Read-only.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    return posFailure(
      "INTERNAL_ERROR",
      toErrorMessage(error, "Internal server error - Failed to load counter view"),
      { status: 500 }
    );
  }
}
