...
import { buildGuestsView } from "@/lib/guests/buildGuestsView";
import { posFailure, posSuccess, toErrorMessage } from "@/app/api/_lib/pos-envelope";

export const runtime = "nodejs";

/**
 * GET /api/guests/view?locationId=<uuid>
 * Returns full guests read model for the location. Read-only.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    return posFailure(
      "INTERNAL_ERROR",
      toErrorMessage(error, "Internal server error - Failed to load guests view"),
      { status: 500 }
    );
  }
}
